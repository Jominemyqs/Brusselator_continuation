function features = feature_evaluation_right(featpar, modelpar)
%FEATURE_EVALUATION  Feature extractor.
% For featpar.feature == 'spiral', returns rV (spacetime reflection symmetry)
% computed from V(t,x) after relaxing from ic0.
%
% rV_full(V) = ||V - fliplr(V)||_F / ||V||_F
% Optionally window to last fraction of time samples (default 25%).
...
if strcmp(featpar.feature,'spiral')
    ...
    % ---------------- parameters ----------------
    par = struct;
    par.sigma = modelpar.a;   % a = sigma
    par.b     = modelpar.b;

    % Relax time (match continuation)
    ...
    % Observation time (for spacetime metric)
    % Recommend: 1200 for stable rV separation; you can lower for speed.
    ...
    % Windowing settings (match your diagnostics default: last 25%)
    ...
    % IMPORTANT: we need V for rV
    [~, ~, V] = solve_brusselator_1d(ic_rel, par, T_obs, 0);

    % --- REGULARITY GATE ---
    if ~check_temporal_coherence(V)
    ...
    % If V didn't come back (unexpected), fall back to final-profile metric
    if isempty(V)
        u_final = ic_rel(:,2);
        denom = norm(u_final);
        if denom < eps
    ...
    if denom < eps
        ...
        features = norm(Vuse - fliplr(Vuse), 'fro') / denom;
    end

    return
end

% ---------------- otherwise: your original logic ----------------
