...
%CONTINUATION  Predictor-corrector continuation for feature-defined curves.
%
% The corrector geometry is selected through modelpar.corrector_mode:
...
% Store accepted points as columns. Legacy runs include the start twice;
% middle-branch runs can omit approximate seed points from reported results.
include_start = true;
if isfield(contpar,'include_start') && ~isempty(contpar.include_start)
    include_start = logical(contpar.include_start);
end
if include_start
...
if ismember(lower(corrector_mode), {'vertical_middle','vertical','b'})
    aux.prev_crossing = p_current(2);
else
    aux.prev_crossing = p_current(1);
end
aux.prev_drop = aux.prev_crossing; % backward-compatible field name
aux.used_mode = "";
aux.no_cross_count = 0;

max_steps = Inf;
if isfield(contpar,'max_steps') && ~isempty(contpar.max_steps)
    max_steps = contpar.max_steps;
end

while L_current < contpar.L && i < max_steps
    i = i + 1;
    fprintf('step %d\n', i);
    ...
    if isfield(aux,'last_coordinate_window'), last_win = aux.last_coordinate_window;
    elseif isfield(aux,'last_sigma_window'),  last_win = aux.last_sigma_window;
    end
    if isfield(aux,'last_rLR'),          last_rLR = aux.last_rLR; end
    if isfield(aux,'last_r_thr'),        last_thr = aux.last_r_thr; end
    if isfield(aux,'last_sweep_axis'),   sweep_axis = string(aux.last_sweep_axis); end

    fprintf('  pred=(%.6f, %.6f)  corr=(%.6f, %.6f)  dir=(%.4f, %.4f)\n', ...
        p_pred(1), p_pred(2), p_current(1), p_current(2), direction(1), direction(2));
    fprintf('  mode=%s | axis=%s | previous=%.6f | sweep_hw=%g | evals=%g | rLR=[%.2e, %.2e] thr=%.2e win=[%.6f, %.6f]\n', ...
        used_mode, sweep_axis, aux.prev_crossing, last_hw, last_ctr, last_rLR(1), last_rLR(2), last_thr, last_win(1), last_win(2));
    % --------------------------------------------------------
    ...
    % Stop if outside requested sigma range.
    if isfield(contpar,'sigma_bounds') && ~isempty(contpar.sigma_bounds)
        if p_current(1) < contpar.sigma_bounds(1) || p_current(1) > contpar.sigma_bounds(2)
            fprintf('STOP: sigma_bounds hit (sigma=%.6f outside [%.6f, %.6f])\n', ...
                p_current(1), contpar.sigma_bounds(1), contpar.sigma_bounds(2));
            break;
        end
    end
...
if i >= max_steps
    fprintf('STOP: max_steps=%d reached.\n', max_steps);
end
end
